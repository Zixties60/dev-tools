...
export default function WebhookTestToken() {
  ...
  const [responseHeaders, setResponseHeaders] = useState([{ key: "X-Powered-By", value: "DevTools" }]);
  
  const webhookUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/api/webhook-test/${token}`;
